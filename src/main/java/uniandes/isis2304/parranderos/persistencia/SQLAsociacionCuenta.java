...
class SQLAsociacionCuenta
{
	 ...
	 */
	public SQLAsociacionCuenta (PersistenciaParranderos pp)
	{
		this.pp = pp;
	}
	 ...
	 */
	public long adicionarAsociacionCuenta (PersistenceManager pm, String loginJefe, String loginEmpleado, long cuentaJefe, long cuentaEmpleado, int salario, String frecuenciaPago) 
	{
        Query q = pm.newQuery(SQL, "INSERT INTO " + pp.darTablaAsociacionCuenta () + " values (?, ?, ?, ?, ?, ?)");
        q.setParameters(loginJefe, loginEmpleado, cuentaJefe, cuentaEmpleado, salario, frecuenciaPago);
        return (long) q.executeUnique();
	}
	
	
	public long eliminarAsociacionCuentaPorId (PersistenceManager pm, long idAsociacionCuenta)
	{
        Query q = pm.newQuery(SQL, "DELETE FROM " + pp.darTablaAsociacionCuenta()+ " WHERE ID = ?");
        q.setParameters(idAsociacionCuenta);
        return (long) q.executeUnique();            
	}
	public long reducirSaldo(PersistenceManager pm, long idAsociacionCuenta, int monto) {
		// TODO Auto-generated method stub
		Query q = pm.newQuery(SQL, "UPDATE A_PRESTAMO SET SALDO = SALDO - ? " +"WHERE ID = ?");
        q.setParameters(monto, idAsociacionCuenta);
        return (long) q.executeUnique();
	}
	...
	public boolean verificarCuota(PersistenceManager pm, long idAsociacionCuenta, int monto) {
		// TODO Auto-generated method stub
