...
class SQLAdministrador 
{
	 ...
	 */
	public SQLAdministrador (PersistenciaParranderos pp)
	{
		this.pp = pp;
	}
...
//	 */
	...
	{
		Query q = pm.newQuery(SQL, "SELECT * FROM A_ADMINISTRADOR");
...
//	}
	...
	public Administrador darAdministradorPorLoginClave(PersistenceManager pm, String loginAdmin,
			String claveAdmin) {
		// TODO Auto-generated method stub
		Query q = pm.newQuery(SQL, "SELECT * FROM A_ADMINISTRADOR WHERE login = ?"+" AND palabraclave = ?");
		q.setResultClass(Administrador.class);
		q.setParameters(loginAdmin, claveAdmin);
		return (Administrador) q.executeUnique();
	}

	public long adicionarAdministrador(PersistenceManager pm, String login, String credenciales) {
		// TODO Auto-generated method stub
		 Query q = pm.newQuery(SQL, "INSERT INTO A_ADMINISTRADOR values (?, ?)");
	        q.setParameters(login, credenciales);
	        return (long) q.executeUnique();
	}
